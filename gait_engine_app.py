...
# Preferred: you run your main hardware script (with the latest functions),
# then import/launch this app. Otherwise, set HARDWARE_MODULE to your file's module name.
HARDWARE_MODULE = "main"  # e.g., "my_robot_main" (without .py). Leave None to try __main__ only.

def _resolve_runtime():
    # Try from __main__ (when this file is imported by your already-running script)
    envs = []
    try:
...
class GaitApp:
    ...
    def _build_controls(self):
        ...
        # Gait selector
        self.gait_var = tk.StringVar(value="creep")
        ...
        # Velocity sliders
        self.vx_var = self._mk_scale(panel, "vx", -0.12, 0.12, 0.04, 0, self._on_vx, 0.005)
        ...
        self.hgt_var  = self._mk_param(panel, "step_height", 0.010,0.060,0.030,0.001,2, self._on_step_height)

        # Home button
        btn_home = tk.Button(panel, text="Feet → Home", command=self._home_feet)
        btn_home.grid(row=3, column=0, sticky="we", padx=4, pady=6)
    ...
    def _mk_scale(self, parent, label, a, b, init, col, cb, res):
        fr = tk.Frame(parent)
        tk.Label(fr, text=label, width=6, anchor="w").pack(side="left")
    ...
    def _home_feet(self):
        # Stop engine and return feet to home offsets (0,0,0) in body frame
        self.engine.stop()
        self.btn.config(text="Start Gait")
        self._lcd_msg("Feet: Home")
        iklegs_move = self.engine._iklegs_move
        iklegs_move({0:(0,0,0), 1:(0,0,0), 2:(0,0,0), 3:(0,0,0)}, step_multiplier=10, speed=20, delay=0.0)
        self._lcd_clear()
